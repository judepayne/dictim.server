...
(def diag-spec
  {:nodes
   ...
   {:labels
    ...
    :styles
    [[{:fill "'#f4a261'", :border-radius 8}
      [:contains :functions "Accounting"]]]},
   :edge-specs {:labels [[{:key :data-type}]]},
   :container->attrs {"Securities" {:style.fill "'#d6edd5'"}}})


(deftest parsing-and-fixing
  (testing "I can parse the json and fix it up to be a valid diagram spec."
    (is (= (g/fix-diagram-specs (parse-string req-json true))
           diag-spec))))
